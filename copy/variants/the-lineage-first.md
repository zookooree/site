...
*Angle: the inheritance — from the loom that stopped itself, through the floor that developed people before products, to a line whose product is minds with souls.*

---

## Zookooree: The Agent Factory

**We build AI agents on a production line that writes each one a soul before its first line of code, and ships each one with a certificate of trust — evidence attached, expiry date printed.**

A century ago a loom learned to stop itself when a thread broke. We inherited that lesson and aimed it at a new material: minds. For anyone who needs to run an agent and sleep.
...
What breaks gets published. The retros show what failed and what we learned; the first one is already public, three bugs and all.

No certificate pretends to be permanent. Every one expires, and a lapsed certificate is treated as a stopped line, not a quiet week.

Every agent we ship exists to free a human from tedious work, never to replace human judgment. If something we build constrains more than it liberates, we rethink it. No exceptions.
...
**Improve daily — kaizen.** Every agent better than the one before. Every failure feeds back into the system. Every retro feeds forward. The factory is never finished, and that is its deepest feature; a finished factory is a dead one.
...
**Decentralization.** Protocols over bureaucracies. Rules arising from voluntary transactions among free agents, not from a headquarters. What we make belongs to no one and serves everyone; the making happened, and who held the tools is secondary. Skepticism toward concentrated power is not our mood. It is our architecture.
...
The repos are open at [github.com/zookooree](https://github.com/zookooree). Read a retro. Read a soul document. Then pull the thread that looks broken — the line is built to stop.
